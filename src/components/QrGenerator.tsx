...
export default function QrGenerator({
  ...
  if (simple) {
    return (
      <div
        ...
        className={`flex flex-col items-center justify-center p-4 bg-white rounded-2xl border ${isDarkMode ? "border-cyan-500/35 shadow-[0_0_25px_rgba(6,182,212,0.2)]" : "border-slate-100 shadow-inner"
          }`}
      >
        <canvas id="qr-canvas-element" ref={canvasRef} className="w-full max-w-[240px] h-auto object-contain rounded-lg" />
      </div>
    );
  }
  ...
  return (
    <div id="qr-generator-container" className="flex flex-col items-center">
      ...
      <div
        id="qr-card"
        className={`relative w-full max-w-[310px] sm:max-w-[340px] md:max-w-[380px] rounded-3xl p-4 md:p-6 transition-all duration-300 shadow-2xl border ${isDarkMode
      ...
      >
        ...
        {/* User Info Header */}
        <div id="qr-user-header" className="flex items-center gap-3 md:gap-4 mb-3 md:mb-5">
          <div
            id="user-avatar"
            className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex items-center justify-center text-white font-bold bg-gradient-to-br shadow-md ${gradientClass}`}
          >
            {initials}
          </div>
          <div id="user-details" className="text-left flex-1 min-w-0">
            <p className={`text-[9px] md:text-[10px] ${isDarkMode ? "text-cyan-400" : "text-indigo-600"} font-bold uppercase tracking-widest`}>
              Session QR Profile
            </p>
          ...
          {/^\d{6}$/.test(sessionId) ? (
            <div className="mt-2.5 mb-1 flex flex-col items-center gap-0.5 w-full select-none">
              ...
              <div
                ...
                className={`py-1.5 px-3 md:py-2 md:px-4 rounded-lg md:rounded-xl font-mono text-lg md:text-xl font-black tracking-[0.35em] cursor-pointer transition-all border text-center ${codeCopied
                    ? "bg-emerald-500 border-emerald-500 text-white"
                    : isDarkMode
                      ? "bg-slate-950 border-white/10 text-cyan-400 hover:border-cyan-500/50 shadow-inner"
                      : "bg-slate-50 border-slate-200 text-indigo-600 hover:border-indigo-600/50 shadow-inner"
                  }`}
                title="Click to copy code"
              >
                {codeCopied ? "COPIED" : `${sessionId.substring(0, 3)} ${sessionId.substring(3, 6)}`}
              </div>
            </div>
          ) : (
            <p className="text-[9px] text-slate-400 mt-2 mb-1 font-mono break-all select-all font-bold uppercase tracking-wider text-center">
        ...
        {/* Action Buttons */}
        <div id="qr-actions" className="flex flex-col gap-2 md:gap-3 mt-3 md:mt-5">
          <button
            id="btn-copy-link"
            onClick={handleCopyLink}
            className={`flex items-center justify-center gap-1.5 md:gap-2 w-full py-2.5 md:py-3 px-3 md:px-4 rounded-xl font-bold transition-all duration-200 cursor-pointer text-[10px] md:text-xs uppercase tracking-wider hover:scale-[1.01] ${copied
          ...
          <div id="sub-actions-grid" className="grid grid-cols-2 gap-3">
            ...
            <button
              ...
              onClick={() => {
                if (navigator.share) {
                  navigator.share({
                    title: "E2E Instant Chat Pairing",
